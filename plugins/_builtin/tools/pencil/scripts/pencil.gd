...
const pencil_shader_file: RDShaderFile = preload("../resources/pencil.glsl")

# !! MUST match value used in shader program !!
var shader_size = 32
var rd_groups: int

var brush_shader = rd.shader_create_from_spirv(pencil_shader_file.get_spirv())
...
func on_pointer_move(mouse_pos: Vector2) -> void:
	current_mouse_pos = mouse_pos.floor()
